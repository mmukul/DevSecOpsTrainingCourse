    ...
    stage ('Vulnerability Scan') {
      steps {
        parallel(
          ...
          "Trivy Scan":{
              sh "docker run --rm aquasec/trivy:0.35.0 --exit-code 0 --severity HIGH docker.io/webgoat/webgoat"
              sh "docker run --rm aquasec/trivy:0.35.0 --exit-code 0 --severity CRITICAL docker.io/webgoat/webgoat"
          }
        )
       }
    }
